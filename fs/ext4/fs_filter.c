...
#ifdef ZTE_FEATURE_ONLY_PREVENT_SHELL_USE_RESERVEDSPACE
static unsigned int fuse_euid = 0;
#endif
...
#ifdef ZTE_FEATURE_ONLY_PREVENT_SHELL_USE_RESERVEDSPACE
	if (fuse_euid == 0) {
		if (strstr(current->comm, "rs.media.module")) {
			fuse_euid = cur_euid.val;
			pr_err("zte special:here initialize fuse_euid to %u.\n",fuse_euid);
		}
	}
	/*if (cur_euid.val == 2000 || cur_euid.val == 0) {
		pr_err("zte special: not allow uid_shell.");
		return false;
	} else */
	if (fuse_euid != 0 && cur_euid.val == fuse_euid) {
		//pr_err("zte special: not 2 allow uid_fuse.");
		return false;
	} else {
		return true;
	}
