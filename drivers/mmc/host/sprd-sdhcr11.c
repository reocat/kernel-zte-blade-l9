...
static void sprd_reset_ios(struct sprd_sdhc_host *host)
{
	...
	if ((strcmp(host->device_name, "sdio_nm") == 0)) {
...
static int sprd_sdhc_set_vqmmc(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	case MMC_SIGNAL_VOLTAGE_330:
		 ...
		 * so we must return
		 */
		if ((strcmp(host->device_name, "sdio_emmc") == 0) || (strcmp(host->device_name, "sdio_nm") == 0)) {
			err = -EINVAL;
			goto out;
		}
		mmiowb();
	...
	case MMC_SIGNAL_VOLTAGE_180:
		...
		if ((strcmp(host->device_name, "sdio_nm") == 0)) {
			dev_err(dev, "rain.zhang 2 sdio_nm\n");
			if (host->flags & SPRD_PINCTRL_SWITCH_VOLTAGE) {
				err = pinctrl_select_state(
					host->pinctrl, host->pins_uhs);
				if (err)
...
{
	u32 tmp_value = 0;
	if(NULL != host->adi_ioaddr){
		tmp_value = readl_relaxed(host->adi_ioaddr);
		tmp_value &= ~(host->adi_ch_en_mask);
		writel_relaxed(tmp_value, host->adi_ioaddr);
	}
	...
	struct sprd_sdhc_host *host)
{
	...
	ret = of_property_read_u32_array(np, "adi-addr", adi_addr, 3);
	if (!ret){
		host->adi_ioaddr = ioremap(adi_addr[0]+adi_addr[1], 256);
		host->adi_ch_en_mask = adi_addr[2];
	}
		...
		struct sprd_sdhc_host *host)
{
	...
	if ((strcmp(host->device_name, "sdio_sd") == 0) || (strcmp(host->device_name, "sdio_nm") == 0)) {
...
static int sprd_sdhc_probe(struct platform_device *pdev)
{
	...
	host->regulator_enabled = false;
	host->adi_ioaddr = NULL;
	host->adi_ch_en_mask = 0;
	...
	sprd_reset_ios(host);
	if ((strcmp(host->device_name, "sdio_nm") == 0)) {
	ret = gpio_request_one(128+39, GPIOF_OUT_INIT_HIGH, "GPIOID");
	if (ret < 0) {
		dev_err(dev, "gpio39 request failed %d\n", ret);
		gpio_set_value(128+39, 1);
	}

	ret = gpio_request_one(128+88, GPIOF_OUT_INIT_LOW, "GPIOID");
	if (ret < 0) {
		dev_err(dev, "gpio88 request failed %d\n", ret);
