...
int zte_mmc_regulator_set_ocr(struct mmc_host *mmc,
			struct regulator *supply, int value,
			unsigned short vdd_bit)
{
	int			result = 0;
	if (vdd_bit) {

		result = regulator_set_voltage(supply, value, value);
		if (result == 0 && !mmc->regulator_enabled) {
			result = regulator_enable(supply);
			if (!result)
				mmc->regulator_enabled = true;
...
/**
 *	mmc_judge_card_type - judge NM or SIM card which plugin
 *	@host: host which changed state.
 *	@delay: optional delay to wait before detection (jiffies)
 *
 *	added by rain.zhang
 */
static int _mmc_judge_card_type(struct mmc_host *host)
{
	...
	struct pinctrl_state *pinctrl_state0,*pinctrl_state1,*pinctrl_state2,*pinctrl_state3;
	char *s0 = "conf0";
	char *s1 = "conf1";
	char *s2 = "conf2";
	char *s3 = "conf3";
	...
	struct sprd_sdhc_host *host_sprd = mmc_priv(host);
	printk("zte: _mmc_judge_card_type enter-------\n");

	pdev = host_sprd->pdev;
	if (pdev == NULL) {
		printk(KERN_EMERG"%s, get pdev failed!\n", __func__);
	}

	if (!IS_ERR(host->supply.vmmc))
		{ printk("zte: set vddscore to 1800v -----\n");
				zte_mmc_regulator_set_ocr(host_sprd->mmc,
					host->supply.vmmc, 1800000,1);}
	mmc_delay(1);

	printk(KERN_EMERG "this is NM card detect pinctrl -----\n");
	p = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(p)) {
		dev_err(&pdev->dev, "zte Can't find pinctrl\n");
			goto out;
	}

	pinctrl_state0 = pinctrl_lookup_state(p, s0);
	if (IS_ERR(pinctrl_state0)) {
		printk(KERN_EMERG "%s, pinctrl lookup state for '%s' failed(%ld)!\n",
			__func__, s0,PTR_ERR(pinctrl_state0));
	}

	ret0 =  pinctrl_select_state(p, pinctrl_state0);
	if(ret0 < 0) {
		printk(KERN_EMERG"gpio149 pinctrl -----%d\n",ret0);
	}

	pinctrl_state2 = pinctrl_lookup_state(p, s2);
	if (IS_ERR(pinctrl_state2)) {
		printk(KERN_EMERG "%s, pinctrl lookup state for '%s' failed(%ld)!\n",
		__func__, s2, PTR_ERR(pinctrl_state2));
	}
	ret2 =  pinctrl_select_state(p, pinctrl_state2);

	/*detect which card that plugin*/
	gpio_request(128+39, "GPIOID");
	gpio_request(128+88, "GPIOID");
	gpio_request(128+149, "GPIOID");

	gpio_direction_input(128+149);
	value = gpio_get_value(128+149);

	printk(KERN_EMERG "detect voltage is %d-----\n", value);
	pinctrl_state3 = pinctrl_lookup_state(p, s3);
	if (IS_ERR(pinctrl_state3)) {
		printk(KERN_EMERG "%s, pinctrl lookup state for '%s' failed(%ld)!\n",
		__func__, s3, PTR_ERR(pinctrl_state3));
	}
	ret3 =  pinctrl_select_state(p, pinctrl_state3);

	/*switch back to sd0_sd2 */

	pinctrl_state1 = pinctrl_lookup_state(p, s1);
	ret1 =  pinctrl_select_state(p, pinctrl_state1);
	if(ret1 < 0) {
		printk(KERN_EMERG "gpio149 switch to sd0_sd2 failed %d\n",ret1);
	}
	gpio_free(128+149);

	printk("detect which card that plugin value is %x\n", value);
	flag = value;

	if (!IS_ERR(host->supply.vmmc))
		{ printk("zte:  vddscore power off -----\n");
				zte_mmc_regulator_set_ocr(host_sprd->mmc,
					host->supply.vmmc, 3300000, 0);
		}

	if(flag == 1)
	{
		/*do sim card setting here*/
		printk(KERN_EMERG "do sim card setting here\n");
		#ifndef ZTE_FEATURE_SINGLE_SIM
		gpio_set_value(128+39, 0);
		gpio_set_value(128+88, 1);
		#endif
/******************************************************************
		v1 = gpio_get_value(128+39);
		v2 = gpio_get_value(128+88);
		printk(KERN_EMERG "gpio39 88 %d %d-----\n", v1, v2);
*******************************************************************/
		ret_value = 0;
	}
	else
	{
		printk(KERN_EMERG "do nm card setting here");
...
void mmc_rescan(struct work_struct *work)
{
	...
	struct sprd_sdhc_host *host_sprd;
	int i;
	 ...
	 */
	mmc_bus_put(host);
	host_sprd = mmc_priv(host);
	if (strcmp(host_sprd->device_name, "sdio_nm") == 0)//only do this for nm card
		if (host->ops->get_cd &&host->ops->get_cd(host) == 1) {
		if(0 == _mmc_judge_card_type(host)) {
			/*0´ú±ísim¿¨£¬1´ú±ínm¿¨*/
			mmc_claim_host(host);
			mmc_power_off(host);
			printk("zte: _mmc_judge_card_type is sim card,power off-----\n");
			mmc_release_host(host);
			goto out;
		}
	}
...
void mmc_start_host(struct mmc_host *host)
{
	...
	{	mmc_claim_host(host);
		mmc_power_up(host, host->ocr_avail);
		dev_err(mmc_dev(host), "mmc_start_host:mmc_power_up enter\n");
		mmc_release_host(host);
	}
	}
