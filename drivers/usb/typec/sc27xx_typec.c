...
#define SC27XX_RP_380UA             (BIT(2)|BIT(3))
...
static int sc27xx_typec_connect(struct sc27xx_typec *sc, u32 status)
{
	...
	case SC27XX_AUDIO_CABLE:
		...
		break;
	default:
		dev_info(sc->dev, "return[line=%d]: %s()\n", __LINE__, __func__);
		return 0;
	}
	...
	dev_info(sc->dev, "[line=%d]: %s() ->: sc->state=%d\n", __LINE__, __func__, sc->state);
	switch (sc->state) {
	case SC27XX_ATTACHED_SNK:
	case SC27XX_DEBUG_CABLE:
		sc->pre_state = SC27XX_ATTACHED_SNK;
		ret = extcon_set_state_sync(sc->edev, EXTCON_USB, true);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	case SC27XX_ATTACHED_SRC:
		sc->pre_state = SC27XX_ATTACHED_SRC;
		if (!IS_ERR(sc->gpiod) && gpiod_get_value(sc->gpiod))
			return 0;
		ret = extcon_set_state_sync(sc->edev, EXTCON_USB_HOST, true);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	case SC27XX_AUDIO_CABLE:
		sc->pre_state = SC27XX_AUDIO_CABLE;
		ret = extcon_set_state_sync(sc->edev, EXTCON_JACK_HEADPHONE, true);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	default:
		break;
...
static void sc27xx_typec_disconnect(struct sc27xx_typec *sc, u32 status)
{
	...
	dev_info(sc->dev, "[line=%d]: %s() ->: sc->pre_state=%d\n"
		, __LINE__, __func__, sc->pre_state);
	switch (sc->pre_state) {
	case SC27XX_ATTACHED_SNK:
	case SC27XX_DEBUG_CABLE:
		ret = extcon_set_state_sync(sc->edev, EXTCON_USB, false);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	case SC27XX_ATTACHED_SRC:
		ret = extcon_set_state_sync(sc->edev, EXTCON_USB_HOST, false);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	case SC27XX_AUDIO_CABLE:
		ret = extcon_set_state_sync(sc->edev, EXTCON_JACK_HEADPHONE, false);
		dev_info(sc->dev, "[line=%d]: %s() ->: ret=%d\n", __LINE__, __func__, ret);
		break;
	default:
		break;
...
static irqreturn_t sc27xx_typec_interrupt(int irq, void *data)
{
	...
	if (ret)
		goto clear_ints;
	dev_info(sc->dev, "[line=%d]: %s() ->: "
	       "event=0x%x, sc->state=0x%x\n", __LINE__, __func__, event, sc->state);
