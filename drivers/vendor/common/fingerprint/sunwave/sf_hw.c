...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    if (IS_ERR(ctl_dev->vdd_reg)) {
        ...
        sf_debug(ERR_LOG, "Regulator get failed from dts, err=%d\n", err);
        return err;
    } else {
        sf_debug(INFO_LOG, "Regulator get success from dts\n");
    }
    ...
    if (err < 0) {
        ...
        sf_debug(INFO_LOG, "%s:Power voltage[%d] get success from dts\n",
