...
static int gf_fingerprint_power_init(struct gf_dev *gf_dev)
{
	...
	gf_debug(DEBUG_LOG, "regulator_vddcammot");
	gf_dev->fp_reg = regulator_get(dev, "vddcammot");
#else
	gf_debug(DEBUG_LOG, "regulator_vfp");
...
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	if (rc < 0) {
		...
		gf_debug(INFO_LOG, "%s:Power type[%d] get success from dts\n",
	...
	if (gf_dev->power_type == 1) {
		...
		if (rc < 0) {
			...
			gf_debug(INFO_LOG, "%s:Power voltage[%d] get success from dts\n",
