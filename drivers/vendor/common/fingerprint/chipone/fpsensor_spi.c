...
static int fpsensor_fingerprint_power_init(fpsensor_data_t *fpsensor_dev)
{
    ...
    fp_debug(DEBUG_LOG, "regulator_vddcammot");
    fpsensor_dev->fp_reg = regulator_get(dev, "vddcammot");
#else
    fp_debug(DEBUG_LOG, "regulator_vfp");
...
static int fpsensor_get_tyep_power(fpsensor_data_t *fpsensor_dev)
{
    ...
    if (ret < 0) {
        ...
        fp_debug(INFO_LOG, "%s:Power type[%d] get success from dts\n",
    ...
    if (ret < 0) {
        ...
        fp_debug(INFO_LOG, "%s:Power voltage[%d] get success from dts\n",
