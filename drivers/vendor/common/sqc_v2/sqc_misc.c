...
static int zte_screen_on_set(const char *val, const void *arg)
{
	...
	sqc_notify_daemon_changed(SQC_NOTIFY_SCREEN_ON,
	...
	union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
			retval = get_client_vote(misc_hal_data.fcc_votable, SQC_THERMAL_SETTING_VOTER);
			    ...
			    size_t count, loff_t *f_pos)
{
	...
	case SQC_IFS_PTL_COM_WAKELOCK:
		pr_info("write sqc_wake_lock: %d\n", handle_buf[0]);
		if (handle_buf[0]) {
			wake_lock_timeout(&misc_hal_data.sqc_wakelock, 16 * HZ);
		} else {
			wake_unlock(&misc_hal_data.sqc_wakelock);
		}
		break;
	default:
		retval = -ENOMEM;
...
static int sqc_misc_init(void)
{
	...
	wake_lock_init(&misc_hal_data.sqc_wakelock, WAKE_LOCK_SUSPEND, "sqc_wakelock_hal");
...
static void sqc_misc_exit(void)
{
	...
	wake_lock_destroy(&misc_hal_data.sqc_wakelock);
