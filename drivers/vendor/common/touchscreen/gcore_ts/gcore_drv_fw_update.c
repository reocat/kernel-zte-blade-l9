...
int gcore_update_prepare(u8 *fw_buf)
{
	...
	GTP_DEBUG("flash op data:%x %x %x %x %x %x", update_info.flash_op_data[0], update_info.flash_op_data[1],
		  update_info.flash_op_data[2], update_info.flash_op_data[3], update_info.flash_op_data[4],
...
/* gcore_reset();  //move to gcore_enter_idm_mode() */
	...
	if (!wait_for_completion_timeout(&gcore_fw_update_complete, 10 * HZ)) {
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
	...
	GTP_DEBUG("fw buf:%x %x %x %x %x %x", fw_buf[0], fw_buf[1], fw_buf[2], fw_buf[3], fw_buf[4], fw_buf[5]);
 ...
 */
static ssize_t fw_mode_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 mode = 0;
 ...
 */
static ssize_t fw_reg_rw_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 addr = 0;
	u32 value = 0;
	u32 length = 0;
	...
	if (!strncmp(buf, "-w", 2)) {
		...
		if (!strncmp(buf, "addr=", 5) && !strncmp(buf + 14, "len=", 4) &&
			...
			if (gcore_fw_read_reg(addr, r_buff, length)) {
				GTP_ERROR("fw reg write fail");
				return -EPERM;
			}
 ...
 */
static ssize_t fw_update_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
...
static ssize_t hostdl_idm_set_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int enabled = 0;
	u8 read_data[4] = { 0 };
