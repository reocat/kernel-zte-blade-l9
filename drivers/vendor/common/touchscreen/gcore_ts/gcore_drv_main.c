	...
	if (coor_data[0] == 0xB1) {
		...
		if (!sum) {
			if (!gdev->tp_suspend) {
				...
				queue_delayed_work(gdev->fwu_workqueue, &gdev->fwu_work, msecs_to_jiffies(1000));
			}
...
static irqreturn_t tpd_eint_interrupt_handler(unsigned int irq, void *p)
{
	struct gcore_dev *gdev = (struct gcore_dev *)p;
	unsigned long flags;
...
static s32 gcore_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (get_tp_chip_id() == 0) {
		if ((tpd_fw_cdev.tp_chip_id != TS_CHIP_MAX) && (tpd_fw_cdev.tp_chip_id != TS_CHIP_GCORE)) {
