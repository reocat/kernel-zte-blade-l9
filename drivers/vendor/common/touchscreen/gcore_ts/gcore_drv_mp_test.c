...
int gcore_parse_mp_test_dt(struct gcore_mp_data *mp_data)
{
	...
	if (mp_data->test_chip_id) {
		of_property_read_u8_array(np, MP_TEST_OF_TEST_CHIP_ID, mp_data->chip_id, sizeof(mp_data->chip_id));
	}
...
int gcore_parse_mp_test_ini(struct gcore_mp_data *mp_data)
{
	...
	buff = read_line(buff, buff_len, f);
	GTP_DEBUG("ini read line %s", buff);
	ret = sscanf(buff, "open_cb=%d", &mp_data->open_cb);
	if (ret < 0)
	...
	buff = read_line(buff, buff_len, f);
	GTP_DEBUG("ini read line %s", buff);
	ret = sscanf(buff, "open_min=%d", &mp_data->open_min);
	if (ret < 0)
	...
	buff = read_line(buff, buff_len, f);
	GTP_DEBUG("ini read line %s", buff);
	ret = sscanf(buff, "short_cb=%d", &mp_data->short_cb);
	if (ret < 0)
	...
	buff = read_line(buff, buff_len, f);
	GTP_DEBUG("ini read line %s", buff);
	ret = sscanf(buff, "short_min=%d", &mp_data->short_min);
	if (ret < 0)
...
int gcore_mp_test_int_pin(struct gcore_mp_data *mp_data)
{
	...
	GTP_DEBUG("reg addr: 0xC0000098 read_buf: %x %x %x %x", read_buf[0], read_buf[1], read_buf[2], read_buf[3]);
...
int gcore_mp_test_item_open(struct gcore_mp_data *mp_data)
{
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (node_data < mp_data->open_min) {
			GTP_ERROR("node_data %d < open_min %d test fail!", i, mp_data->open_min);
...
int gcore_mp_test_item_short(struct gcore_mp_data *mp_data)
{
	...
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
		...
		if (node_data < mp_data->short_min) {
			GTP_ERROR("node_data %d < short_min %d test fail!", i, mp_data->short_min);
...
int gcore_mp_test_fn_init(struct gcore_dev *gdev)
{
	...
	mp_data->mp_proc_entry = proc_create_data(MP_TEST_PROC_FILENAME, S_IRUGO, NULL, &gcore_mp_fops, mp_data);
