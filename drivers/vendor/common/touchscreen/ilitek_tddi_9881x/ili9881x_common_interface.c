...
static void ili9881x_reset_cmd_display_rotation(void)
{
	u8 edge_palm_para = 0;
...
void ili_touch_reset_cmd_and_clean_point(enum ilitek_cmd cmd)
{
	ili_tp_reset();
	...
	case HEADSET:
		...
		if (ilits->display_rotation) {
			msleep(20);
			ili9881x_reset_cmd_display_rotation();
		}
		break;
	case CHARGER:
...
static int ili9881x_set_headset_state(struct tpd_classdev_t *cdev, int enable)
{
	...
	if (!ilits->tp_suspend) {
		ili_touch_reset_cmd_and_clean_point(HEADSET);
	}
...
static int ili9881x_set_display_rotation(struct tpd_classdev_t *cdev, int mrotation)
{
	...
	ili_touch_reset_cmd_and_clean_point(MROTATION);
...
static void ili9881x_work_charger_detect_work(struct work_struct *work)
{
	...
	if (!ilits->tp_suspend && (ilits->charger_mode != charger_mode_old)) {
		ili_touch_reset_cmd_and_clean_point(CHARGER);
...
static int ili9881x_data_request(s16 *frame_data_words, enum tp_test_type  test_type)
{
	...
	if (test_type == RAWDATA_TEST) {
		for (i = 4, index = 0; index < row * col + FW_TP_STATUS_BUFFER; i += 2, index++) {
			if (index < row * col)
				frame_data_words[index] = ((data[i] << 8) + data[i + 1]) / 10;
			else
