...
#ifdef GET_VENDOR_NAME_BY_LCDINFO
	u32 flash_fw_vendor_id = 0, hex_vendor_id = 0;
#endif
...
#ifdef GET_VENDOR_NAME_BY_LCDINFO
		hex_vendor_id = (tfd.new_fw_cb >> 24) & 0xFFFF;
		flash_fw_vendor_id = (ilits->chip->fw_ver >> 24) & 0xFFFF;
		ILI_INFO("hex_vendor_id = 0x%x,flash_fw_vendor_id = 0x%x\n", hex_vendor_id, flash_fw_vendor_id);
		if (hex_vendor_id != flash_fw_vendor_id) {
			ILI_INFO("need fw upgrade\n");
			return UPDATE_FAIL;
		}
#endif
	}
