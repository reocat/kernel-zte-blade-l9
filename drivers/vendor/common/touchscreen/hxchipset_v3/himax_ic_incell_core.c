...
int hx_0f_op_file_dirly(char *file_name)
{
	...
	if (adb_upgrade_firmware != NULL) {
		if (adb_fw_entry == NULL) {
			adb_fw_entry = kzalloc(sizeof(struct ts_firmware), GFP_KERNEL);
			if (adb_fw_entry == NULL) {
				E("Request from file alloc struct firmware failed");
				goto error_request_adb_firmware;
			}
		}
		I("%s, Entering,file adb_upgrade_firmware\n", __func__);
		adb_fw_entry->data = adb_upgrade_firmware->data;
		adb_fw_entry->size =  adb_upgrade_firmware->size;
		goto firmware_upgrade;
	}
error_request_adb_firmware:
...
firmware_upgrade:
	if ((adb_upgrade_firmware != NULL) && (adb_fw_entry != NULL))
		g_core_fp.fp_bin_desc_get((unsigned char *)adb_fw_entry->data, HX1K);
	else
	...
	if ((adb_upgrade_firmware != NULL) && (adb_fw_entry != NULL)) {
		g_core_fp.fp_firmware_update_0f(adb_fw_entry);
		if (ret >= 0)
