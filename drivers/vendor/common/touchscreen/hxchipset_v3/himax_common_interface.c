...
char himax_vendor_name[20] = { 0 };
#ifdef HX_ZERO_FLASH
struct firmware *adb_fw_entry = NULL;
...
int himax_get_fw_by_lcminfo(void)
{
	...
	memset(private_ts->CTPM_firmware_name, 0, sizeof(private_ts->CTPM_firmware_name));
	memset(private_ts->hx_criteria_csv_name, 0, sizeof(private_ts->hx_criteria_csv_name));
#ifdef HX_ZERO_FLASH
	memset(private_ts->himax_default_firmware_name, 0, sizeof(private_ts->himax_default_firmware_name));
	memset(private_ts->himax_mp_firmware_name, 0, sizeof(private_ts->himax_mp_firmware_name));
#endif
	...
	for (i = 0; i < ARRAY_SIZE(himax_vendor_l); i++) {
		if (strnstr(panel_name, himax_vendor_l[i].vendor_name, strlen(panel_name))) {
			...
			strlcpy(himax_vendor_name, himax_vendor_l[i].vendor_name,
				sizeof(himax_vendor_name));
			ret = 0;
			goto out;

		}
	}
	strlcpy(himax_vendor_name, "Unknown", sizeof(himax_vendor_name));
	ret = -EIO;
out:
	snprintf(private_ts->CTPM_firmware_name, sizeof(private_ts->CTPM_firmware_name),
		"Himax_firmware_%s.bin",himax_vendor_name);
	snprintf(private_ts->hx_criteria_csv_name, sizeof(private_ts->hx_criteria_csv_name),
		"hx_criteria_%s.csv", himax_vendor_name);
	I("HXTP firmware name :%s\n", private_ts->CTPM_firmware_name);
#ifdef HX_ZERO_FLASH
	snprintf(private_ts->himax_default_firmware_name, sizeof(private_ts->himax_default_firmware_name),
		"%s_%s.bin", HXTS_DEFAULT_FIRMWARE, himax_vendor_name);
	snprintf(private_ts->himax_mp_firmware_name, sizeof(private_ts->himax_mp_firmware_name),
		"Himax_mp_firmware_%s.bin", himax_vendor_name);
#endif
	return ret;
...
static int himax_tp_fw_upgrade(struct tpd_classdev_t *cdev, char *fw_name, int fwname_len)
{
	...
	I("%s: upgrade from file(%s) start!\n", __func__, fileName);
	adb_upgrade_firmware = himax_tp_requeset_firmware(fileName);
	if (adb_upgrade_firmware == NULL) {
...
int himax_save_failed_node(int failed_node)
{
	...
	E("himax_save_failed_node enter.");
...
{
	memset(private_ts->g_hx_save_file_path, 0, sizeof(private_ts->g_hx_save_file_path));
	snprintf(private_ts->g_hx_save_file_path, sizeof(private_ts->g_hx_save_file_path), "%s", buf);

	I("save file path:%s.", private_ts->g_hx_save_file_path);
...
{
	memset(private_ts->g_hx_save_file_name, 0, sizeof(private_ts->g_hx_save_file_name));
	snprintf(private_ts->g_hx_save_file_name, sizeof(private_ts->g_hx_save_file_name), "%s", buf);

	I("save file path:%s.", private_ts->g_hx_save_file_name);
...
static int tpd_test_save_file_name_show(struct tpd_classdev_t *cdev, char *buf)
{
	...
	num_read_chars = snprintf(buf, PAGE_SIZE, "%s\n", private_ts->g_hx_save_file_name);
...
void himax_tpd_register_fw_class(void)
{
	...
	snprintf(private_ts->g_hx_save_file_path, sizeof(private_ts->g_hx_save_file_path), "%s", HX_RSLT_OUT_PATH);
	snprintf(private_ts->g_hx_save_file_name, sizeof(private_ts->g_hx_save_file_name), "%s", HX_RSLT_OUT_FILE);
}
