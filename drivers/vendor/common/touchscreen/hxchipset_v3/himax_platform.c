...
const struct spi_geni_qcom_ctrl_data spi_ctrdata = {
	.spi_cs_clk_delay = 10,
				...
				unsigned long event, void *data)
{
	...
	if (evdata->data && event == DRM_PANEL_EARLY_EVENT_BLANK && ts && ts->dev) {
		blank = evdata->data;
		switch (*blank) {
...
#else
	if (evdata->data && event == DRM_PANEL_EVENT_BLANK && ts && ts->dev) {
		blank = evdata->data;
		switch (*blank) {
		case DRM_PANEL_BLANK_UNBLANK:
...
#ifdef CONFIG_SPI_QCOM_GENI
	/* new usage of QCOM spi API */
	memcpy(&ts->spi_ctrl, &spi_ctrdata, sizeof(struct spi_geni_qcom_ctrl_data));
