...
static int i_get_FW(void)
{
	...
	if (ret < 0) {
		...
		I("%s: request firmware %s fail, try request firmware: %s.\n",
			__func__,   BOOT_UPGRADE_FWNAME,   private_ts->himax_default_firmware_name);
		ret = request_firmware(&hxfw,  private_ts->himax_default_firmware_name,  private_ts->dev);
		if (ret < 0)
#endif
		{
		...
		if (valid) {
			...
			if (!finger_down[i] && (ts->nFinger_support <= HIMAX_MAX_TOUCH_NUM)) {
				...
				I("%s:touch down id: %d, coord [%d:%d]\n", __func__,
					i, g_target_report_data->p[i].x, g_target_report_data->p[i].y);
			}
#ifdef HX_REPORT_BY_ZTE_ALGO
#ifdef HX_PRESSURE_ENABLE
			tpd_touch_press(ts->input_dev, g_target_report_data->p[i].x, g_target_report_data->p[i].y,
							i, g_target_report_data->p[i].w, g_target_report_data->p[i].w);
#else
			tpd_touch_press(ts->input_dev, g_target_report_data->p[i].x, g_target_report_data->p[i].y,
...
#elif defined(HX_CONFIG_FB) || defined(HX_CONFIG_DRM)
