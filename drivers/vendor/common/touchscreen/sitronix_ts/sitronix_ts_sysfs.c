...
static ssize_t st_inform_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	sret = snprintf(buf, 1000,
			...
			stx_gpts.ts_dev_info.x_res, stx_gpts.ts_dev_info.y_res, stx_gpts.ts_dev_info.chip_id);
	return sret;
}
...
static ssize_t st_forceUpg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	snprintf(spath, sizeof(spath), "%s", buf);
...
static ssize_t st_smart_wakeup_switch_store(struct device *dev, struct device_attribute *attr, const char *buf,
					    size_t count)
{
	STX_DEBUG("%s", __func__);
	mutex_lock(&stx_gpts.dev_mutex);
...
static ssize_t st_temporary_set_i2caddr_store(struct device *dev, struct device_attribute *attr, const char *buf,
					      size_t count)
{
	char addr;
...
ssize_t sitronix_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
	...
	if (ret < 0) {
		STX_ERROR("sitronix_write write  error (%d)", ret);
	}
	kfree(tmp);
	return ret;
}
...
int st_dev_node_init(void)
{
	...
	STX_INFO("%s,%d", __func__, __LINE__);
