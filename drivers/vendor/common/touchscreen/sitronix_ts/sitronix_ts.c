...
static inline void sitronix_ts_handle_sensor_key(struct input_dev *input_dev,
						 struct sitronix_sensor_key_t *key_array, char *pre_key_status,
...
static void sitronix_ts_work_func(struct work_struct *work)
{
	...
	sitronix_ts_handle_sensor_key(ts->input_dev, sitronix_sensor_key_array, &sitronix_sensor_key_status, buf[0],
				      (sizeof(sitronix_sensor_key_array) / sizeof(struct sitronix_sensor_key_t)));
...
static irqreturn_t sitronix_ts_irq_handler(int irq, void *dev_id)
{
	...
	if (stx_gpts.irq_is_enable == 0) {
		spin_unlock_irqrestore(&stx_gpts.irq_lock, flags);
		STX_ERROR("%s irq_is_enable is %d", __func__, stx_gpts.irq_is_enable);
		return IRQ_HANDLED;
	}
	stx_gpts.irq_is_enable = 0;
...
static int sitronix_parse_dt(struct device *dev, struct sitronix_ts_platform_data *pdata)
{
	int rc;
	struct device_node *np = dev->of_node;
	struct property *prop;

	STX_DEBUG("%s,%d", __func__, __LINE__);

	pdata->name = "sitronix";

	/* reset, irq gpio info */
	pdata->reset_gpio = of_get_named_gpio_flags(np, "sitronix,reset-gpios", 0, &pdata->reset_gpio_flags);
	STX_INFO("%s,pdata->reset_gpio=%d", __func__, pdata->reset_gpio);

	if (pdata->reset_gpio < 0)
		return pdata->reset_gpio;

	pdata->irq_gpio = of_get_named_gpio_flags(np, "sitronix,interrupt-gpios", 0, &pdata->irq_gpio_flags);
	STX_INFO("%s,pdata->irq_gpio=%d", __func__, pdata->irq_gpio);
	...
	if (prop) {
		...
		rc = of_property_read_u32_array(np, "sitronix,button-map", pdata->button_map, pdata->num_button);
		if (rc) {
...
static int sitronix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif
	ret = sitronix_ts_input_dev_init(&stx_gpts);
	if (ret) {
	...
	if (stx_gpts.client->irq) {
		...
		/* ret = request_threaded_irq(stx_gpts.client->irq, NULL, sitronix_ts_irq_handler,
			stx_gpts.irq_flags, client->name, &stx_gpts); */
		ret =
		    request_irq(stx_gpts.client->irq, sitronix_ts_irq_handler, stx_gpts.irq_flags, client->name,
				&stx_gpts);
		if (ret < 0) {
	...
	STX_DEBUG("%s,line=%d", __func__, __LINE__);
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && sitronix_data && sitronix_data->client) {
		blank = evdata->data;
...
static int __init sitronix_ts_init(void)
{
	...
	STX_INFO("IIC ADD DRIVER %s,ret_iic=%d", __func__, ret_iic);
