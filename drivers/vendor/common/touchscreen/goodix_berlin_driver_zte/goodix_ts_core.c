	...
	struct goodix_ts_event *ts_event)
{
	...
	if (ts_event->request_code == REQUEST_TYPE_CONFIG)
		...
		ts_err("can not handle request type 0x%x",
			  ts_event->request_code);
	if (ret)
		ts_err("failed handle request 0x%x",
			 ts_event->request_code);
	else
		ts_err("success handle ic request 0x%x",
			  ts_event->request_code);
	return ret;
}
...
int goodix_send_ic_config(struct goodix_ts_core *cd, int type)
{
	...
	ts_info("goodix_send_ic_config enter");
...
static int goodix_later_init_thread(void *data)
{
	...
	if (ret) {
		...
		ts_info("success init fw update module");
	}
