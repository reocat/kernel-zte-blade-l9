...
//extern int goodix_send_ic_config(struct goodix_ts_core *cd, int type);
extern int goodix_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin);
...
static int goodix_fw_version_compare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (ret) {
		ts_err("failed get active fw version");
		return -EINVAL;
	}
...
static int goodix_send_flash_cmd(struct goodix_flash_cmd *flash_cmd)
{
	...
	ts_err("flash cmd status error %d, ack 0x%x, status 0x%x, ret %d",
		i, tmp_cmd.ack, tmp_cmd.status, ret);
	if (ret) {
		ts_err("reason: bus or paltform error");
		return -EINVAL;
	}
...
static int gtp_cfg_bin_upgrade(struct goodix_ts_core *core_data, const struct ts_firmware *gtp_firmware)
{
	...
	ret = goodix_read_cfg_bin(&core_data->pdev->dev, cfg_bin);
	if (!ret) {
		ts_info("read cfg bin SUCCESS");
	} else {
		ts_err("read cfg bin FAILED");
		goto exit;
	}
