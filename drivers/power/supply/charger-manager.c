...
#if defined(CONFIG_CHARGER_SC2720) || defined(CONFIG_CHARGER_SC2721)
static int cm_adapt_plug_check(struct charger_manager *cm)
{
	struct charger_desc *desc = cm->desc;
	...
	dev_info(cm->dev, "charge_vol=%d batt_uV=%d bat_uA=%d\n",
				charge_vol, batt_uV, bat_uA);
	if ((bat_uA < 0) && (charge_vol <= batt_uV) && is_ext_pwr_online(cm)) {
		dev_info(cm->dev, "usb adapt plug but vchg is high, disable charging\n");
#ifdef CONFIG_CHGING_WITH_VOTER
		vote(cm->policy.battery_charging_enabled_votable, OTHER_SETTING_VOTER,
											true, 0);
#endif
...
#ifdef ZTE_FEATURE_PV_AR
		if (cm->desc->psy_hardware2 && !enable)
			charger_set_prop_by_name(cm->desc->psy_hardware2,
